...
Данное приложение нигде в реальной жизни не используется, создавалось ради учебы, и получения практических знаний. В первую очередь работой с базой данных и EFCore.
Однако проводились консультации с работником школы, чтобы понять, что и где нужно.

Проект построен на базе WinForms.

.NetCore 3.1 - понимание, что выбор для WinForms неудачный, т.к. .NetCore считается кроссплатформенным решением, а WinForms кроме как на Windosw нигде не запускается пришло позже. но не остановило от дальнейшей разработки. Цель научится работать с EntityFrameworkCore.
...
Кнопка WORD - выводит конкретно вбыранную строку в word файл, на рабочий стол.
